...
const EditOrAddOrder = () => {
    ...
    const handleItemAction = useCallback((item, action) => {
        setTemporaryOrder(prevOrder => {
            ...
            return prevOrder.reduce((acc, curr) => {
                ...
                return newQuantity > 0 ? [...acc, { ...curr, quantity: newQuantity }] : acc;
            }, []);
        });
    }, []);
    ...
    useEffect(() => {
        if (existingOrder) {
            const orderDetails = typeof existingOrder.order_details === 'string' ? JSON.parse(existingOrder.order_details) : existingOrder.order_details;

            const initialOrder = orderDetails.map(detail => {
                const menuItem = menu.menuItems.find(mi => mi.menu_item_id === detail.menu_item_id);
                return {
